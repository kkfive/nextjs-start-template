...
Motion (Framer Motion) React 动画库指南
...
提供 Motion 动画库的使用指南，包括手势交互、滚动动画、布局动画、性能优化和 Next.js 集成。
...
### ❌ 不使用 Motion
- 简单列表动画 → 使用 `auto-animate`（3.28 KB vs 34 KB）
- 静态内容
- 3D 动画 → 使用 Three.js
...
function ParallaxSection() {
  ...
  return <motion.div style={{ y }}>视差内容</motion.div>
}
```
...
## Motion vs AutoAnimate

| 方面 | AutoAnimate | Motion |
|------|-------------|--------|
| 包大小 | 3.28 KB | 2.3-34 KB |
| 手势 | ❌ | ✅ |
| 滚动 | ❌ | ✅ |
| 列表 | ✅ 自动 | ✅ 手动 |

**经验法则**: 90% 用 AutoAnimate，10% 用 Motion
