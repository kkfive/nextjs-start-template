 ...
 */
export async function callScenario(client: BizClient, scenario: ScenarioType) {
 ...
 */
export async function callEnvelopeScenario(client: BizClient, scenario: ScenarioType) {
  const res = await client.example.request.scenario.$post({ json: { scenario } })
  return res.json()
}
