...
3. **业务调用**：通过 `@kkfive/rpc` 的 calls，rpc 实例从该 app 的 `@/service/*`（`rpc-server`）注入
...
import { createMaterial } from '@kkfive/rpc'
...
export async function createMaterialAction(formData: FormData) {
  ...
  const created = await createMaterial(rpcServer, parsed.data)
...
- [ ] 业务通过 `@kkfive/rpc` 的 calls（经 `@/service/rpc-server`），不直接拼 URL
