...
✅ packages/rpc → @kkfive/contracts(type), @kkfive/ui, @kkfive/http-client
✅ packages/rpc → apps/api 的 AppType（type-only only，端到端类型链）
...
```
✅ apps/{app}/src/app/ → @kkfive/rpc, @kkfive/ui, @/components/*, @/lib/*
✅ apps/{app}/src/components/ → @kkfive/ui, @kkfive/rpc, @/lib/*
✅ apps/{app}/domain/ → @kkfive/rpc（re-export + 注入运行时）
...
- **实例由 app 注入**：HttpService 抽象在 `@kkfive/http-client`，具体实例由各 app 创建并注入 rpc 的 hc client
