...
- 新建共享包 / 新建应用 → 本 skill 的目录与依赖规则，并先检索现有同类结构
...
monorepo:
  ...
  internal/     工具链配置（tsconfig / tailwind-config / nextjs-config）
...
| ❌ 不要 | ✅ 应该 |
|---|---|
| `packages/*` 里 import `apps/*` | 共享包不依赖应用；AppType 由 app service type-only 消费 |
...
- 根 `AGENTS.md`：新建共享包/应用时的检索门禁与三层边界
