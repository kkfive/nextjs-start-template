...
页面应仅组合 `src/features/<feature>/` 的入口、路由元数据和 Next.js 路由能力。可复用的跨 feature 视图下沉到该 app 的 `src/components`；业务视图、calls、hooks、状态和纯逻辑留在所属 feature。请求实例（HttpService）与 hc RPC 客户端的运行时适配放在该 app 的 `src/service`；跨 app 的自有 API calls 才进入 `@kkfive/rpc`。
