...
依赖流向单向：`packages/` 不可依赖任何 `apps/`；`apps/` 之间不可互相依赖；`internal/` 不可依赖 `apps/` 或 `packages/`，仅装构建工具链。`packages/contracts` 和 `packages/utils` 零运行时依赖；`packages/domain-core` 禁止依赖 React / Next.js / Hono；`packages/http-client` 禁止依赖运行环境。
